...
Normal_Run='SRR6277192'
Tumor_Run='SRR6277190'
results='/scratch/kh31516/Pan_cancer/HSA/results/P1' #
reference='/work/szlab/Lab_shared_PanCancer/source' 
dataN='/scratch/kh31516/Pan_cancer/HSA/data/P1/${Normal_Run}'
dataT='/scratch/kh31516/Pan_cancer/HSA/data/P1/${Tumor_Run}'
...
MuTect_out='/scratch/kh31516/Pan_cancer/HSA/Mutect/P1'
MuTect2_out='/scratch/kh31516/Pan_cancer/HSA/Mutect2/P1'
Germline_out='/scratch/kh31516/Pan_cancer/HSA/Germline/P1'
DepthOfCoverage='/scratch/kh31516/Pan_cancer/HSA/DepthOfCoverage/P1'
strelka_out='/scratch/kh31516/Pan_cancer/HSA/strelka/'
...
--out ${MuTect_out}/P1_rg_added_sorted_dedupped_removed.bam_call_stats.txt \
--coverage_file ${MuTect_out}/P1_rg_added_sorted_dedupped_removed.bam_coverage.wig.txt \
--vcf ${MuTect_out}/P1_rg_added_sorted_dedupped_removed.MuTect.vcf

####### Annovar #######
# Extract PASS records from vcf
module load Perl/5.26.1-GCCcore-6.4.0

awk '$7 == "PASS" {print $0}' ${MuTect_out}/P1_rg_added_sorted_dedupped_removed.MuTect.vcf > ${MuTect_out}/P1_rg_added_sorted_dedupped_removed.MuTect.vcf-PASS

### Sanger 5 steps filtering ###
# 5 Steps filtering
grep -w KEEP ${MuTect_out}/P1_rg_added_sorted_dedupped_removed.bam_call_stats.txt | cut -f1,2,26,27,38,39 > ${MuTect_out}/P1_PASS.stat

python $script/Filter_MutectStat_5steps.py ${MuTect_out}/P1_PASS.stat ${MuTect_out}/P1_rg_added_sorted_dedupped_removed.MuTect.vcf-PASS
# annovar input preparation
perl $annovar_index/convert2annovar.pl -format vcf4old ${MuTect_out}/P1_rg_added_sorted_dedupped_removed.MuTect.vcf-PASS > ${MuTect_out}/P1_rg_added_sorted_dedupped_removed.MuTect.vcf-PASS-avinput

# annovar annotate
perl $annovar_index/annotate_variation.pl --buildver canFam3 ${MuTect_out}/P1_rg_added_sorted_dedupped_removed.MuTect.vcf-PASS_filteredMut-avinput $annovar_index

# add gene name
python $script/Add_GeneName_N_Signature.py ${MuTect_out}/P1_rg_added_sorted_dedupped_removed.MuTect.vcf-PASS_filteredMut-avinput.exonic_variant_function ${reference}/Canis_familiaris.CanFam3.1.99.chr.gtf_geneNamePair.txt
...
--f1r2-tar-gz ${MuTect2_out}/P1-f1r2.tar.gz \
-L ${MuTect2_source}/Uniq-Canis_familiaris.CanFam3.1.99.gtf-chr1-38X-CDS-forDepthOfCoverage.interval.list \
-O ${MuTect2_out}/P1_MuTect2_GATK4_noDBSNP.vcf


## pass this raw data to LearnReadOrientationModel:
gatk LearnReadOrientationModel -I ${MuTect2_out}/P1-f1r2.tar.gz -O ${MuTect2_out}/read-orientation-model.tar.gz

# filtered Mutect2 files

gatk FilterMutectCalls -R ${reference}/canFam3.fa \
-ob-priors ${MuTect2_out}/read-orientation-model.tar.gz \
-V ${MuTect2_out}/P1_MuTect2_GATK4_noDBSNP.vcf \
-O ${MuTect2_out}/filtered-P1_MuTect2_GATK4_noDBSNP.vcf
...
${MuTect2_out}/filtered-P1_MuTect2_GATK4_noDBSNP.vcf \
${MuTect2_out}/DbSNP_filtered-P1_MuTect2_GATK4.vcf
...
${reference}/DbSNP_canFam3_version151-DogSD_Feb2020_V4.vcf \
${MuTect2_out}/DbSNP_filtered-P1_MuTect2_GATK4.vcf \
${MuTect2_out}/PON_DbSNP_filtered-P1_MuTect2_GATK4.vcf
...
#awk '$7 == "PASS" {print $0}' ${MuTect2_out}/PON_DbSNP_filtered-P1_MuTect2_GATK4.vcf > ${MuTect2_out}/PON_DbSNP_filtered-P1_MuTect2_GATK4.vcf-PASS

# 5 Steps filtering
python $script/Mutect2_5Steps_filtering.py \
${MuTect2_out}/PON_DbSNP_filtered-P1_MuTect2_GATK4.vcf \
${MuTect2_out}/PON_DbSNP_filtered-P1_MuTect2_GATK4.vcf \
${MuTect2_out}/P1_VAF_Before.txt \
${MuTect2_out}/P1_VAF_After.txt
...
rm ${MuTect_out}/P1_rg_added_sorted_dedupped_removed.MuTect.vcf-PASS ${MuTect_out}/P1_rg_added_sorted_dedupped_removed.MuTect.vcf-PASS-avinput
rm ${results}/*.bai
...
rm ${results}/*.sai
rm ${MuTect2_out}/DbSNP_filtered-P1_MuTect2_GATK4.vcf
rm ${MuTect2_out}/read-orientation-model.tar.gz
rm ${MuTect2_out}/P1-f1r2.tar.gz
"
